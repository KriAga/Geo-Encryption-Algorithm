...
class Point
{
  ...
  public:
    Point(int intX, int floatX, int intY, int floatY)
    {
        this->intX = intX;
        this->floatX = floatX;
        this->intY = intY;
...
class Encryption
{
  ...
  public:
    vector<Point> encryptByteArray(unsigned int bytes[], int size)
    {
        string blank = "";
        vector<Point> points;
...
#pragma omp parallel for
  ...
  public:
    unsigned int *decryptToByteArray(vector<Point> encrypted)
    {
        int FPPB = 32;
